...
import height from '../attributedString/height';
import sliceAtOffset from '../attributedString/sliceAtOffset';
...
const applyDefaultValues = R.applySpec({
  path: R.prop('path'),
...
const layoutParagraph = engines => (paragraph, lineRect) => {
  const style = paragraph.runs[0].attributes;

  // Guess the line height using the full line before generating fragment rects.
  const lineHeight = height(sliceAtOffset(lineRect.width, paragraph));
  const fragmentRects = generateFragments(lineRect, lineHeight);

  // Perform line breaking
  const wrappingWidths = fragmentRects.map(rect => rect.width);
  const lines = engines.linebreaker(paragraph, paragraph.syllables, wrappingWidths, style);
  ...
  const layoutContainer = container => {
    ...
    while (nextParagraph) {
      ...
      if (paragraphRect.height >= block.height) {
        blocks.push(block);
        paragraphRect = copyRect(paragraphRect);
