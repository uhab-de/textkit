...
describe('attributeString prepend operator', () => {
  ...
  test('should prepend glyph on single run string', () => {
    ...
    expect(R.pluck('id', result.runs[0].glyphs)).toEqual([76, 76, 111, 114, 101, 109]);
    expect(R.pluck('xAdvance', result.runs[0].positions)).toEqual([10, 5, 6, 7, 8, 9]);
  });
  ...
  test('should prepend glyph on multiple run string', () => {
    ...
    expect(R.pluck('xAdvance', result.runs[0].positions)).toEqual([10, 5, 6, 7]);
